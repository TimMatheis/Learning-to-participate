...
# ---------------------------------------- NEW ----------------------------------------------
def reward_participation(obs, a_others, config, n_recipients=1,
               output_nonlinearity=tf.nn.sigmoid):
    """Computes shares traded
    ...
    h2 = tf.layers.dense(inputs=h1, units=config.n_hr2,
                         activation=tf.nn.relu,
                         use_bias=True, name='reward_h2')
    share_traded_out = tf.layers.dense(inputs=h2, units=1,
                                 activation=output_nonlinearity,
                                 use_bias=False, name='reward')
    return share_traded_out 
# -------------------------------------------------------------------------------------------
