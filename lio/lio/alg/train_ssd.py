...
def train_function(config):
    ...
    list_suffix = ['given', 'received', 'reward_env',
                   ...
                   'r_riverside', 'r_beam', 'r_cleared', 'share_0'] # , 'share_0' ------------------------
    ...
    while idx_episode < n_episodes:
        ...
        if idx_episode % period == 0:
            (given, received, reward_env, reward_total,
             ...
             r_cleared, share_0) = evaluate.test_ssd(n_eval, env, sess, list_agents) # share_0 --------------------------
            ...
            for idx in range(env.n_agents):
                s += ','
                s += '{:.2e},{:.2e},{:.2e},{:.2e},{:.2f},{:.2e},{:.2e},{:.2e},{:.2e}'.format( # ------------- ,{:.2e} ------------------
                    *combined[:, idx])
...
def run_episode(sess, env, list_agents, epsilon, prime=False):
    ...
    # SCENARIO -> SET ONE ON TRUE
    no_sharing = False
    sc_half_2 = False
    sc_half_3 = False
    predecision = False # for two agents
    predecision_3 = False
    pot_3 = True

    list_buffers = [Buffer(env.n_agents) for _ in range(env.n_agents)]
    list_obs = env.reset() # ---------------------- ADDED: , share_0 ---------------
    done = False

    budgets = np.zeros(env.n_agents)

    # ADDED: activated for predecision
    if predecision_3 or predecision:
        list_actions = []
        list_binary_actions = []
        for agent in list_agents:
    ...
    if pot_3:
        ...
        for idx, buf in enumerate(list_buffers):
            buf.add([list_obs[idx], list_actions[idx], env_rewards[idx],
                        list_obs_next[idx], done, share_0[idx]]) #--- added:, share_0[idx] ----------------------
            buf.add_r_from_others(total_reward_given_to_each_agent[idx])
            if env.obs_cleaned_1hot:
                buf.add_action_all(list_binary_actions)
            else:
    ...
    while not done:
        ...
        if predecision:
            env_rewards[0] = share_0* env_rewards[0] + (1-share_0)*env_rewards[1]
            env_rewards[1] = share_0* env_rewards[1] + (1-share_0)*env_rewards[0]
