...
def train(config):
    ...
    if config.env.name == 'er':
        list_suffix = ['reward_total', 'n_lever', 'n_door',
                       'received', 'given', 'r-lever', 'r-start', 'r-door', 'share_0'] # --------------------- ADDED: , 'share_0' --------------------------------
    elif config.env.name == 'ipd':
        list_suffix = ['given', 'received', 'reward_env',
                       'reward_total', 'share_0'] # --------------------- ADDED: , 'share_0' --------------------------------
        ...
        if idx_episode % period == 0:
            ...
            if config.env.name == 'er':
                (reward_total, n_move_lever, n_move_door, rewards_received,
                 ...
                 r_start, r_door, share_0) = evaluate.test_room_symmetric(               # --------------------- ADDED: , share_0 --------------------------------
                     n_eval, env, sess, list_agents)
                matrix_combined = np.stack([reward_total, n_move_lever, n_move_door,
                                            rewards_received, rewards_given,
                                            r_lever, r_start, r_door, share_0])              # --------------------- ADDED: , share_0 --------------------------------
            elif config.env.name == 'ipd':
                given, received, reward_env, reward_total, share_0 = evaluate.test_ipd(  # --------------------- ADDED: , share_0 --------------------------------
                    n_eval, env, sess, list_agents)
            ...
            for idx in range(env.n_agents):
                ...
                if config.env.name == 'er':
                    s += ('{:.3e},{:.3e},{:.3e},{:.3e},{:.3e},'
                          '{:.3e},{:.3e},{:.3e},{:.3e}').format(                       #  ----------------------- ADDED: ,{:.3e} ------------------------------------------
                              *matrix_combined[:, idx])
                elif config.env.name == 'ipd':
                    s += '{:.3e},{:.3e},{:.3e},{:.3e},{:.3e}'.format(               # ----------------------- ADDED: ,{:.3e} ------------------------------------------
                        *matrix_combined[:, idx])
            if config.env.name == 'er':
                s += ',%.2f\n' % steps_per_episode
            else:
...
def run_episode(sess, env, list_agents, epsilon, prime=False):
    ...
    while not done:
        list_actions = []
        for agent in list_agents:
            action = agent.run_actor(list_obs[agent.agent_id], sess, # -------------------------- ADDED: , trade ----------------------
                                     epsilon, prime)
            #trade = agent.run_actor(list_obs[agent.agent_id], sess,     # -------------------------- ADDED ----------------------
            #                epsilon, prime)
        ...
        if env.name == 'er':
            ...
            list_obs_next, env_rewards, done, share_0 = env.step(list_actions, share_0)   #--- added:, share_0 (two times!) --------------------------------------------------
        
        for idx, buf in enumerate(list_buffers):
            buf.add([list_obs[idx], list_actions[idx], env_rewards[idx],
                     list_obs_next[idx], done, share_0])    #--- added:, share_0 ----------------------
...
class Buffer(object):
    ...
    def add(self, transition):
        ...
        self.share_0.append(transition[5]) #--- added ----------------------
