...
class LIO(object):
    ...
    def __init__(self, config, dim_obs, l_action, nn, agent_name,
        ...
        self.can_give = False # False # changed! was 'True'
        self.participate = False # added, but not used...
        ...
        with tf.variable_scope(self.agent_name):
            with tf.variable_scope('policy_main'):
                with tf.variable_scope('policy'):
                    ...
                    # NEW: ----------------------------------------------
                    # probs_trade = actor_net(self.obs, self.t_action, self.nn)
                    # ----------------------------------------------------
                ...
                # NEW: ----------------------------------------------
                # self.probs_trade = (1 - self.epsilon) * probs_trade + self.epsilon / self.t_action
                # self.log_probs_trade = tf.log(self.probs_trade)
                # self.action_samples_trade = tf.multinomial(self.log_probs_trade, 1)
                # ----------------------------------------------------
            ...
            with tf.variable_scope('policy_prime'):
                ...
                # NEW: -----------------------------------------------------------------------
                # self.probs_prime_trade = (1-self.epsilon)*probs_trade + self.epsilon/self.t_action
                # self.log_probs_prime_trade = tf.log(self.probs_prime_trade)
                # self.action_samples_prime_trade = tf.multinomial(self.log_probs_prime_trade, 1)
                # ------------------------------------------------------------------------------
    ...
    def run_actor_trade(self, obs, sess, epsilon, prime=False):
        feed = {self.obs: np.array([obs]), self.epsilon: epsilon}
        if prime:
            action = sess.run(self.action_samples_prime, feed_dict=feed)[0][0]
        else:
