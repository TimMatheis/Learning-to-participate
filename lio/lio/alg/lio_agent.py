...
class LIO(object):
    ...
    def __init__(self, config, l_obs, l_action, nn, agent_name,
        ...
        self.can_give = False # ------------------------------- CHANGED, was True -------------------
        self.participate = False # ----------------------------------------- ADDED ------------------
        ...
        with tf.variable_scope(self.agent_name):
            with tf.variable_scope('policy_main'):
                ...
                with tf.variable_scope('eta'):
                    ...
                    # ------------------------------------ Only idea --------------------------------------------------------
                    #self.reward_function = networks.reward_participation(self.obs, self.action_others,
    ...
    def run_actor(self, obs, sess, epsilon, prime=False):
        ...
        if prime:
            ...
            # trade = sess.run(self.action_samples_prime, feed_dict=feed)[0][0] # --------- ADDED
        else:
            action = sess.run(self.action_samples, feed_dict=feed)[0][0]    # --------- ADDED: []
            # trade = sess.run(self.action_samples, feed_dict=feed)[0][0]  # --------- ADDED
        return action #, trade
    ...
    # ----------------------- NEW ---------------------------------------------------------------------------------------
    def trade_participation(self, obs, action_all, sess, share_0): # added share_0
        action_others_1hot = util.get_action_others_1hot(action_all, self.agent_id,
                                                         self.l_action)
        feed = {self.obs: np.array([obs]),
                self.action_others: np.array([action_others_1hot]),
                self.share_0: share_0}                              # added share_0
        reward = sess.run(self.reward_function, feed_dict=feed)
        reward = reward #.flatten()  # * self.r_multiplier
