...
def test_ipd(n_eval, env, list_agents, config):
    """Eval episodes on IPD."""

    n_c = np.zeros((n_eval, config.env.n_agents))  # count of cooperation
    n_d = np.zeros((n_eval, config.env.n_agents))  # count of defection
    rewards_env = np.zeros((n_eval, config.env.n_agents))
    rewards_total = np.zeros((n_eval, config.env.n_agents))
    ...
    for idx_episode in range(1, n_eval + 1):
        ...
        print("Trade actions", trade_actions)
        ...
        while not done:
            ...
            for idx, agent in enumerate(list_agents):
                ...
                action = agent.run_actor_basic(list_obs[idx], epsilon)
                list_actions.append(action)
                if action == 0:
    ...
    rewards_env = np.average(rewards_env, axis=0) / (env.max_steps + 1)
    rewards_total = np.average(rewards_total, axis=0) / env.max_steps

    av_share_0 = av_share / (n_eval * (env.max_steps + 1))
    print("av_share", av_share_0)
    av_share_0 = [av_share_0, av_share_0]

    return rewards_env, av_share_0
